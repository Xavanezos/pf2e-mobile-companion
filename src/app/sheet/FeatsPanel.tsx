...
export function FeatsPanel({ view, onShowDetail }: { view: CharacterView; onShowDetail: (id: string) => void }) {
      ...
      {view.featGroups.map((g) => (
        <section key={g.key}>
          ...
          <div className="divide-y divide-zinc-800">
            {g.feats.map((f) => (
              <button key={f.id} onClick={() => onShowDetail(f.id)} className="flex w-full items-center gap-2 px-3 py-2 text-left">
